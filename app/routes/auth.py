...
@auth_bp.route('/api/seed-menu', methods=['POST'])
@csrf.exempt
def seed_menu():
    """
    Endpoint temporal: crea menú completo para velziaoficial.
    ELIMINAR después de usar.
    """
    import secrets as _secrets
    from app.models import db, Restaurant, Category, Product, User
    ...
    existing_cats = Category.query.filter_by(restaurant_id=restaurant.id).count()
    if existing_cats > 0:
        return jsonify({'error': 'menu already seeded', 'categories': existing_cats}), 409
    ...
    cats_data = [
        ...
        ('Bebidas Calientes', 'Café, tinto yChocolate', 6),
        ('Postres', 'Dulces tradicionales y colombianos', 7),
    ]

    cats = {}
    for name, desc, order in cats_data:
        cat = Category(restaurant_id=restaurant.id, name=name, description=desc, sort_order=order)
        db.session.add(cat)
        db.session.flush()
        cats[name] = cat
    ...
    products = [
        ...
        ('Empanadas Colombianas x3', 'Empanadas de carne con hogao, servidas con ají', 8500, 'Entradas y Para Picar', True, False, True),
        ('Patacones con Hogao', 'Plátano verde frito doble cocción con hogao de tomate y cebolla', 9500, 'Entradas y Para Picar', True, False, False),
        ('Chicharrón con Arepa', 'Chicharrón crujiente con arepa antioqueña y limón', 12000, 'Entradas y Para Picar', False, False, False),
        ('Marranitas', 'Bolitas de plátano verde rellenas de chicharrón, estilo Chocó', 10000, 'Entradas y Para Picar', False, False, False),
        ('Aborrajados', 'Tajadas de plátano maduro rellenas de queso, rebozadas y fritas', 9000, 'Entradas y Para Picar', True, False, False),
        ('Arepa con Queso', 'Arepa de maíz con queso derretido y mantequilla', 6000, 'Entradas y Para Picar', True, False, False),
        ('Buñuelos Colombianos', 'Buñuelos de maíz y queso, crujientes por fuera y esponjosos por dentro', 5000, 'Entradas y Para Picar', True, False, False),
        ('Empanadas de Pollo x3', 'Empanadas de pollo desmechado con especias', 8500, 'Entradas y Para Picar', True, False, False),

        # ── Sopas Calientes ──
        ('Sancocho de Gallina', 'Sancocho tradicional con gallina criolla, yuca, plátano, mazorca y cilantro', 18000, 'Sopas Calientes', False, False, True),
        ('Ajiaco Santafereño', 'Ajiaco bogotano con tres tipos de papa, guascas, mazorca y pollo', 17000, 'Sopas Calientes', False, False, False),
        ('Sancocho de Pescado', 'Sancocho de pescado fresco con leche de coco y plátano', 19000, 'Sopas Calientes', False, False, False),
        ('Caldo de Costilla', 'Caldo reconfortante de costilla de res con papa y cilantro', 12000, 'Sopas Calientes', False, False, False),
        ('Sopa de Arroz con Pollo', 'Sopa espesa de arroz con pollo desmechado y verduras', 13000, 'Sopas Calientes', False, False, False),

        # ── Platos Fuertes ──
        ('Bandeja Paisa', 'Frijoles, arroz, carne molida, chicharrón, huevo frito, plátano maduro, arepa y aguacate', 28000, 'Platos Fuertes', False, False, True),
        ('Arroz con Pollo', 'Arroz amarillo con pollo, verduras y cerveza, servido con patacón', 22000, 'Platos Fuertes', False, False, False),
        ('Sudado de Res', 'Carne de res guisada en salsa de tomate con papa, plátano verde y arroz', 23000, 'Platos Fuertes', False, False, False),
        ('Pollo Sudado', 'Pollo en salsa criolla con arroz, papa y plátano', 20000, 'Platos Fuertes', False, False, False),
        ('Lomo al Trapo', 'Lomo de res envuelto en sal cocido al carbón, con papa criolla y ají', 32000, 'Platos Fuertes', False, False, False),
        ('Punta de Anca en Salsa', 'Punta de anca en salsa de champiñones con arroz y ensalada', 29000, 'Platos Fuertes', False, False, False),
        ('Mojarra Frita', 'Mojarra entera frita crispy con patacones, arroz con coco y ensalada', 25000, 'Platos Fuertes', False, False, False),
        ('Chuleta Valluna', 'Chuleta de cerdo empanizada al estilo valluna con arroz, ensalada y patacón', 24000, 'Platos Fuertes', False, False, False),
        ('Bandeja de Montañera', 'Carne asada, pollo a la plancha, arroz, frijoles, tajadas y ensalada', 30000, 'Platos Fuertes', False, False, False),
        ('Seco de Chivo', 'Chivo guisado estilo Caribe con arroz con coco y plátano maduro', 26000, 'Platos Fuertes', False, False, False),
        ('Trucha con Patacón', 'Trucha en salsa de limón con patacón y arroz', 27000, 'Platos Fuertes', False, False, False),

        # ── Acompañamientos ──
        ('Patacón Solo', 'Patacón crujiente doble cocción', 4000, 'Acompañamientos', True, False, False),
        ('Tajada de Plátano Maduro', 'Tajada frita de plátano maduro', 3500, 'Acompañamientos', True, False, False),
        ('Arroz Blanco', 'Porción de arroz blanco suelto', 3000, 'Acompañamientos', True, False, False),
        ('Frijoles Colorados', 'Frijoles guisados estilo paisa', 4500, 'Acompañamientos', True, False, False),
        ('Ensalada Mixta', 'Lechuga, tomate, zanahoria y aguacate', 5000, 'Acompañamientos', True, False, False),
        ('Aguacate', 'Media unidad de aguacate fresco', 4000, 'Acompañamientos', True, False, False),
        ('Yuca con Mojo', 'Yuca hervida con salsa de ajo y limón', 5500, 'Acompañamientos', True, False, False),

        # ── Bebidas Frías ──
        ('Limonada Natural', 'Limonada fresca con hielo y hierbabuena', 5000, 'Bebidas Frías', True, False, False),
        ('Limonada de Coco', 'Limonada cremosa de coco, refrescante y tropical', 7000, 'Bebidas Frías', True, False, False),
        ('Jugo de Mango', 'Jugo natural de mango colombiano', 5500, 'Bebidas Frías', True, False, False),
        ('Jugo de Guanábana', 'Jugo natural de guanábana', 5500, 'Bebidas Frías', True, False, False),
        ('Jugo de Maracuyá', 'Jugo natural de maracuyá', 5500, 'Bebidas Frías', True, False, False),
        ('Jugo de Corozo', 'Jugo natural de corozo, fruto del Chocó', 6000, 'Bebidas Frías', True, False, False),
        ('Gaseosa Personal', 'Coca-Cola, Sprite o Fanta (330ml)', 3500, 'Bebidas Frías', True, False, False),
        ('Agua Botella', 'Agua pura sin gas (600ml)', 3000, 'Bebidas Frías', True, False, False),
        ('Cerveza Águila', 'Cerveza rubia colombiana bien fría', 4500, 'Bebidas Frías', False, False, False),
        ('Cerveza Póker', 'Cerveza pilsner colombiana', 4500, 'Bebidas Frías', False, False, False),
        ('Cerveza Club Colombia', 'Cerveza premium colombiana dorada', 7000, 'Bebidas Frías', False, False, False),
        ('Coronita', 'Cerveza importada mexicana', 6500, 'Bebidas Frías', False, False, False),
        ('Michelada', 'Cerveza con limón, salsa picante y sal', 8000, 'Bebidas Frías', False, True, False),

        # ── Bebidas Calientes ──
        ('Tinto Colombiano', 'Café tinto tradicional, servido caliente', 2500, 'Bebidas Calientes', True, False, False),
        ('Tinto con Leche', 'Café tinto con leche caliente', 3500, 'Bebidas Calientes', True, False, False),
        ('Cappuccino', 'Espresso con espuma de leche y cacao', 6000, 'Bebidas Calientes', True, False, False),
        ('Chocolate Santafereño', 'Chocolate caliente espeso con queso y arepa', 7000, 'Bebidas Calientes', True, False, False),
        ('Aromática', 'Té de hierbabuena, manzanilla o tilo', 3500, 'Bebidas Calientes', True, False, False),

        # ── Postres ──
        ('Arroz con Leche', 'Arroz con leche tradicional colombiano con canela y coco rallado', 7000, 'Postres', True, False, False),
        ('Torta de Choclo', 'Torta de choclo dulce horneada, estilo tradicional', 8000, 'Postres', True, False, False),
        ('Postre de Natas', 'Dulce cremoso de natas con arepa o galleta', 7000, 'Postres', True, False, False),
        ('Manjar Blanco', 'Dulce tradicional de leche y azúcar, servido frío', 7500, 'Postres', True, False, False),
        ('Gelatina de Pata', 'Gelatina tradicional de colación con coco rallado', 6000, 'Postres', True, False, False),
        ('Cholado', 'Copa de frutas frescas con hielo raspado, leche condensada y salsas', 10000, 'Postres', True, False, False),
        ('Helado Artesanal x2 Bolas', 'Dos bolas de helado artesanal (vainilla, chocolate, fresa o mora)', 6500, 'Postres', True, False, False),
        ('Oblea con Arequipe', 'Oblea rellena con arequipe y queso cremoso', 5500, 'Postres', True, False, False),
    ]

    count = 0
    for name, desc, price, cat_name, veg, spicy, featured in products:
        p = Product(
