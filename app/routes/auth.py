...
@auth_bp.route('/api/seed-sales', methods=['POST'])
@csrf.exempt
def seed_sales():
    """Endpoint temporal: genera ventas simuladas de agosto 2026."""
    import random
    from datetime import datetime, timedelta, timezone
    from app.models import db, Restaurant, Product, Order, OrderItem, User
    ...
    if existing > 0:
        return jsonify({'error': 'orders already exist', 'count': existing}), 409
    ...
    for p in products:
        ...
        if p.is_featured:
            w = 25
        if p.price and p.price < 6000:
            w = 15
        if p.price and p.price > 25000:
            w = 5
        product_weights.append((p, w))
    ...
    names = [
        ...
        'Fernando W.', 'Ana María Z.', 'Ricardo Q.', 'Mónica X.', 'Jorge Y.',
        None, None, None, None, None,
    ]

    phones = [
        '+57 300 123 4567', '+57 310 234 5678', '+57 315 345 6789',
        '+57 320 456 7890', '+57 301 567 8901', '+57 302 678 9012',
        '+57 304 789 0123', '+57 305 890 1234', '+57 311 901 2345',
        '+57 312 012 3456', None, None, None, None, None,
    ]

    payment_methods = ['cash', 'nequi', 'bancolombia', 'card']
    payment_weights = [35, 30, 20, 15]
    ...
    for day in range(1, 32):
        ...
        dow = dt.weekday()
        if dow >= 5:
            base = random.randint(10, 14)
        else:
            base = random.randint(6, 10)
        day_counts[day] = base

    total_target = sum(day_counts.values())

    order_num = 1
    for day in range(1, 32):
        n_orders = day_counts[day]
        for _ in range(n_orders):
            hour_weights = [
                (11, 8), (12, 20), (13, 18), (14, 10),
                (18, 12), (19, 18), (20, 15), (21, 10), (22, 5),
            ]
            hour = random.choices([h for h, _ in hour_weights],
                                  weights=[w for _, w in hour_weights])[0]
            minute = random.randint(0, 59)
            second = random.randint(0, 59)

            created = datetime(2026, 8, day, hour, minute, second, tzinfo=COT)
            created_utc = created.astimezone(timezone.utc)

            pm = random.choices(payment_methods, weights=payment_weights)[0]
            status = random.choices(statuses, weights=[88, 4, 4, 4])[0]

            n_items = random.choices([1, 2, 3, 4], weights=[20, 40, 30, 10])[0]
            chosen = random.choices(product_weights, weights=[w for _, w in product_weights], k=n_items)
            ...
            if pm == 'cash':
                ...
                amount_received = next((b for b in bills if b >= total), total + 5000)
                change_due = amount_received - total

            cust_name = random.choice(names)
            cust_phone = random.choice(phones)

            order = Order(
                restaurant_id=restaurant.id,
                order_number=f'ORD-{order_num:03d}',
                ...
                updated_at=created_utc + timedelta(minutes=random.randint(15, 45)),
            )
    ...
    totals = [o['total'] for o in orders_data]
